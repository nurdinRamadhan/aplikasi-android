-- Locked/suspended wallets still represent pesantren liability. Reconciliation
-- must include their cached saldo; otherwise a security lock can create a false
-- internal mismatch and freeze the whole wallet system.
...
begin
  ...
  where status in ('active', 'locked', 'suspended');
  ...
  set finished_at = now(),
      ...
      details = jsonb_build_object(
        'credit', v_credit,
        'debit', v_debit,
        'cached_balance_statuses', jsonb_build_array('active', 'locked', 'suspended')
      )
  where id = v_run_id;
